...
impl<'a> FatFs<'a> {
    ...
    fn list_directory_raw(
        &mut self,
        dir: Option<FatDirectoryEntry>,
        ...
        let buffer = if let Some(entry) = dir {
            ...
            // list root directory
            assert!(
                (self.root_directory_entry_count * FatDirectoryEntry::BYTES_PER_ENTRY)
                    % self.bytes_per_sector
                    == 0
            );
            let root_sector_count = self.root_directory_entry_count
                * FatDirectoryEntry::BYTES_PER_ENTRY
                / self.bytes_per_sector;
    ...
    pub fn read_file<S: AsRef<str>>(&mut self, path: S) -> Result<Box<[u8]>, &'static str> {
        ...
        if let Some(index) = index {
            ...
            let entry = current_dir
                .iter()
                .find(|entry| entry.name.to_uppercase() == file_token.to_uppercase());
            if let Some(entry) = entry {
                if !entry.is_file() {
                    return Err("not a file");
                }
